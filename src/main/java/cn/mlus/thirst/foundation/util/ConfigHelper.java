...
public class ConfigHelper
{
    private static ResourceLocation parseId(String id)
    {
        ...
        if (colonIndex >= 0)
            return ResourceLocation.fromNamespaceAndPath(id.substring(0, colonIndex), id.substring(colonIndex + 1));
    ...
    public static Map<Item, Number[]> getItemsWithValues(List<? extends List<?>> source)
    {
        ...
        for (List<?> entry : source)
        {
            ...
            {
                Item newItem = ForgeRegistries.ITEMS.getValue(parseId(itemID));
